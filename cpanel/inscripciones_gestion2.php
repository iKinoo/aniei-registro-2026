<?php
	...
	if($_SERVER['REQUEST_METHOD'] == "POST"){
		...
		$strSQL = "DELETE FROM inscripciones WHERE id_usuario = '".$id_usuario."'";
		mysql_query($strSQL);
		for($i=0; $i<count($_POST['talleres']); $i++){
			$id_actividad = $_POST['talleres'][$i];
			$strSQL = "SELECT * FROM inscripciones WHERE id_actividad = '".$id_actividad."' AND id_usuario = '".$id_usuario."'";
			...
			if(mysql_num_rows($registros) == 0){ //Al obtener un CERO significa que no està inscrito a la actividad
				...
				if(mysql_num_rows($registros) < $cupoMAX){ // Verificamos si hay cupo disponible
					...
					$strSQL = "SELECT a.id_actividad, a.nombre, a.hora_inicio, a.hora_final FROM inscripciones i, actividades a WHERE i.id_usuario='".$id_usuario."' AND a.id_actividad=i.id_actividad";
					$registros = mysql_query($strSQL);
					if(mysql_num_rows($registros) == 0){
						...
						while ($registro = mysql_fetch_array($registros, MYSQL_ASSOC)) {
							...
							if(($horaIni <= $hhoraIni && $hhoraIni <= $horaFin) || ($horaIni <= $hhoraFin && $hhoraFin <= $horaFin)){
								$add = false;
								$actividad_no_inscrita[$i] = $registro['nombre'];
					...
					if($add == true){
						...
						$strSQL = "INSERT INTO inscripciones VALUES(null,'".$id_actividad."','".$id_usuario."')";
						mysql_query($strSQL);
		...
		if($_GET['accion'] == "modificar"){	
			$strSQL = "SELECT a.id_actividad, a.nombre, ta.descripcion, i.id_usuario
						FROM actividades a
						INNER JOIN tipoactividad ta ON ta.id_tipoactividad = a.id_tipoactividad
						LEFT JOIN inscripciones i ON i.id_actividad = a.id_actividad AND i.id_usuario = '".$_GET['id_usuario']."'
		...
		if($_GET['accion'] == "ver"){	
			$strSQL = "SELECT a.id_actividad, a.nombre, ta.descripcion, i.id_usuario
						FROM actividades a
						INNER JOIN tipoactividad ta ON ta.id_tipoactividad = a.id_tipoactividad
						INNER JOIN inscripciones i ON i.id_actividad = a.id_actividad AND i.id_usuario = '".$_GET['id_usuario']."'
						WHERE ta.id_tipoactividad < 3 
						ORDER BY a.id_tipoactividad ASC, a.nombre ASC";
		}
...
<? if($_SERVER['REQUEST_METHOD'] == "POST") {
	for($i=0; $i<count($_POST['talleres']); $i++){
		...
		?><div class="mensaje"><?
		if($inscrito[$i] == 0) { ?><h3 align="center"><?php echo $titulo." ".$nombre." ".$apellido; ?><br>ha sido inscrito a<br><?php echo $actividad; ?></h3><? } 
		if($inscrito[$i] == 1) { ?><h3 align="center">Lo sentimos, el cupo para <?php echo $actividad; ?> est&aacute; completo.</h3><? }
		if($inscrito[$i] == 2) { ?><h2 align="center"><?php echo $titulo." ".$nombre." ".$apellido; ?></h2><h3 align="center">ya est&aacute; inscrito en esta actividad.</h3>
<?	} 
		if($inscrito[$i] == 3) { ?>
			...
			<h2 align="center"><?php echo $actividad_no_inscrita[$i]; ?></h2>
	<?	} ?></div><br><? 
	} ?><p align="center"><a href="inscripciones_gestion.php">Regresar</a></p><?
} else { ?>
<form action="<? echo $_SERVER['PHP_SELF']; ?>" method="post" name="form1" id="form1" onSubmit="return ValidaForm(this);">
    ...
    <tr> 
      <td colspan="2"><p><b>Informaci&oacute;n de la Inscripcion </b></p>
    ...
    <tr> 
      <td align="right" valign="top"><p>Participante</p></td> 
      <?php if(isset($_GET['id_usuario'])){
	  			$strSQL = "SELECT nombre, apellido, folio_recibo FROM usuarios WHERE id_usuario='".$_GET['id_usuario']."'";			
				$reg1 = mysql_query($strSQL);
				$reg1 = mysql_fetch_array($reg1, MYSQL_ASSOC);
			} ?>
     <td valign="top"><h2><?php echo HTML(trim($reg1['nombre']))." ".HTML(trim($reg1['apellido'])); ?><br>
       <span class="Estilo" id="nombre"><?php echo $reg1['folio_recibo']; ?></span></h2>
	   <input name="id_usuario" type="hidden" value="<? echo $_GET['id_usuario']; ?>"></td>
    </tr>
    <tr>
   ...
   <? if($_GET['accion'] == "modificar"){ ?><p align="center"><input name="Enviar" type="submit" value="Inscribirme a las actividades seleccionadas" />
     <input name="id_actividades_actuales" type="hidden" value="<? echo $talleres; ?>">
   </p>
   <? } ?>
    <? if($_GET['accion'] == "ver"){ ?><p align="center"><input type="button" name="Submit" value="Modificar inscripciones del participante" onClick="window.location = 'inscripciones_gestion2.php?accion=modificar&id_usuario=<?php echo $_GET['id_usuario']; ?>';">
    </p>
    <? } ?>
  <p align="center"><a href="inscripciones_gestion.php">Regresar</a></p>
</form><? } ?>
