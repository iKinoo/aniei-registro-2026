...
async function main() {
  ...
  console.log('MySQL versión:', version.version);

  const [tz] = await prisma.$queryRaw`SELECT @@session.time_zone as tz, NOW() as now, UTC_TIMESTAMP() as utc`;
  console.log('Zona horaria sesión:', tz.tz);
  console.log('NOW():', tz.now);
  console.log('UTC_TIMESTAMP():', tz.utc);

  const [collation] = await prisma.$queryRaw`SELECT @@collation_database as collation`;
  console.log('Collation BD:', collation.collation);
  ...
  const [row] = await prisma.$queryRaw`SELECT fecha FROM _smoke_test WHERE id = 1`;
  console.log('Fecha insertada:', testDate.toISOString());
  console.log('Fecha leída:', row.fecha.toISOString?.() || row.fecha);

  const diff = Math.abs(testDate.getTime() - new Date(row.fecha).getTime());
  if (diff < 1000) {
